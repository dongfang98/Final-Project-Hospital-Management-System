package com.mecol.hospital.web;

import com.mecol.hospital.entity.Admin;
import com.mecol.hospital.entity.Doctor;
import com.mecol.hospital.entity.Menu;
import com.mecol.hospital.entity.User;
import com.mecol.hospital.service.DoctorService;
import com.mecol.hospital.util.EncryptUtil;
...
public class DoctorController {
    ...
    public ResultUtil login(String username, String password, HttpServletRequest request, HttpSession session)
    {
        ...
        if(doctor!=null)
        {
            ...
            if(depart_id!=0) //说明已经分配科室 可以登录
            {
                ...
                session.setAttribute("hotel_user",doctor);
                session.setAttribute("usertype",3);//代表医生登陆
                session.setAttribute("hotel_user_id",doctor.getDoctor_id());
                session.setAttribute("hotel_user_photo",doctor.getPhoto_path());//获得头像图片名
    ...
    public String info(HttpSession session)
    {
        ...
        Integer doctor_id= (Integer) session.getAttribute("hotel_user_id");
    ...
    public List<Menu> getMenus(HttpSession session)
    {
        ...
        Doctor doctor= (Doctor) session.getAttribute("hotel_user");
